...
Exploration::Exploration(int32_t teamNumber,
                         lcm::LCM* lcmInstance)
        : teamNumber_(teamNumber), state_(exploration_status_t::STATE_INITIALIZING), haveNewPose_(false),
          haveNewMap_(false), haveHomePose_(false), lcmInstance_(lcmInstance), pathReceived_(false) {
...
void Exploration::handleConfirmation(const lcm::ReceiveBuffer* rbuf, const std::string &channel,
                                     const message_received_t* confirm) {
    std::lock_guard<std::mutex> autoLock(dataLock_);
...
int8_t Exploration::executeExploringMap(bool initialize) {
    ...
    */
    ...
    if (!frontiers_.empty()) {
        // check if we're still travelling to a frontier
        auto stillTravellingToFrontier = true;
        // we'll need to recompute if our size is different (meaning there could be a different, closer frontier)
        if (frontiers_.size() != prev_frontier_size) {
            stillTravellingToFrontier = false;
        }

        // if our current path is empty then false
        if (currentPath_.path_length <= 1) {
            stillTravellingToFrontier = false;
        } else {
            // we're close enough to the goal
            const auto d = distance_between_points(Point<double>{currentPose_.x, currentPose_.y}, Point<double>{currentPath_.path.back().x, currentPath_.path.back().y});
            if (d < 0.05) {
                stillTravellingToFrontier = false;
            }
        }
        ...
        // travel to a new one
        if (!stillTravellingToFrontier) {
            // if we're out of frontiers then doing do anything? consider driving with the right hand rule
    ...
    switch (status.status) {
        ...
        default:
            std::cerr
                    << "ERROR: Exploration::executeExploringMap: Set an invalid exploration status. Exploration failed!";
            return exploration_status_t::STATE_FAILED_EXPLORATION;
    }
}
...
int8_t Exploration::executeReturningHome(bool initialize) {
    ...
    */
    ...
    double targetToHome = 0;
    if (currentPath_.path_length > 0) {
        const auto target = currentPath_.path.back();
        targetToHome = distance_between_points(Point<float>(homePose_.x, homePose_.y),
                                               Point<float>(target.x, target.y));
    }
    // recompute path if we reached our current path's goal but our distance to goal is not close
    if (currentPath_.path_length <= 1 ||
        (distToHome > kReachedPositionThreshold && targetToHome > kReachedPositionThreshold)) {
