...
public:
    ...
    */
    mbot_motor_command_t updateCommand(void) {
        ...
        const float kTurnSpeed = 2.0f;//0.3f;
        const float minAllowedTurn = 0.8f;
        // to maintain continuous slowdown
        const float driveSlowdownAlpha = (kDesiredSpeed - kMinSpeed)/std::sqrt(kSlowdownDist);
        ...
        if (!targets_.empty() && !odomTrace_.empty()) {
            ...
            std::cout << "Goal: " << target.x << " " << target.y << " " << target.theta << std::endl; 
            std::cout << "Odometry: " << pose.x << " " << pose.y << " " << pose.theta << " targetHeading: " << targetHeading << std::endl;
            ...
            if (state_ == TURN) {
                if (std::abs(error) > 0.10) //0.10) turn in place until pointed approximately at the target
                {
                    ...
                    turnspeed = std::max(turnspeed, minAllowedTurn);
                    ...
                    state_ = DRIVE;
                       ...
                       DRIVE) // Use feedback to drive to the target once approximately pointed in the correct direction
            {
                ...
                if (distToGoal < kSlowdownDist) {
                    ...
                    speed = kMinSpeed + std::sqrt(distToGoal) * driveSlowdownAlpha;
                }
    ...
    bool timesync_initialized() {
        return timesync_initialized_;
    }
