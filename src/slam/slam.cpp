...
OccupancyGridSLAM::OccupancyGridSLAM(int numParticles,
        ...
        , haveInitializedPoses_(false), waitingForOptitrack_(waitForOptitrack), haveMap_(false), numIgnoredScans_(0),
          filter_(numParticles), map_(10.0f, 10.0f, 0.05f) //30,30,0.1  // create a 10m x 10m grid with 0.05m cells
        , mapper_(5.0f, hitOddsIncrease, missOddsDecrease), lcm_(lcmComm), mapUpdateCount_(0) {
    ...
    std::cout << mappingOnlyMode << ' ' << localizationOnlyMap.length() << std::endl;
    ...
    std::cout << "Before LCM subscription\n";
    ...
    std::cout << "Middle of initialization\n";
...
void OccupancyGridSLAM::runSLAM(void) {
    while (true) {
        ...
        if (isReadyToUpdate()) {
            std::cout << "Before SLAM iteration\n";
...
void OccupancyGridSLAM::handleOdometry(const lcm::ReceiveBuffer* rbuf, const std::string &channel,
...
void
OccupancyGridSLAM::handleOptitrack(const lcm::ReceiveBuffer* rbuf, const std::string &channel, const pose_xyt_t* pose) {
    std::lock_guard<std::mutex> autoLock(dataMutex_);
    optitrackPoses_.addPose(*pose);
...
void OccupancyGridSLAM::runSLAMIteration(void) {
    ...
    if (currentScan_.num_ranges > 100)//250)
    {
        ...
        std::cout << "Before localization update\n";
        updateLocalization();
        updateMap();
            // // check out error error
            // if (!optitrackPoses_.empty()) {
            //     auto a = optitrackPoses_.back();
            //     auto b = currentPose_;
            //     errorTimestamp_.push_back(utime_now());
            //     posError_.push_back(distance_between_points(Point<double>{a.x, a.y}, Point<double>{b.x, b.y}));
            //     headingError_.push_back(angle_diff_abs(a.theta, b.theta));

            //     // calculate average, std, max
            //     const auto& v = posError_;
            ...
            //     std::cout << "Error mean " << mean << " std " << stdev << " max " << *std::max_element(v.begin(), v.end()) << std::endl;
            // } else {
            //     std::cout << "No true pose\n";
            // }
            // const auto elapsedMs = (clock() - startTime)/double(CLOCKS_PER_SEC)*1000;
            // std::cout << "Slam iteration took " << elapsedMs << " ms\n";
    } else {
...
void OccupancyGridSLAM::updateLocalization(void) {
    if (haveMap_ && (mode_ != mapping_only)) {
        previousPose_ = currentPose_;
        currentPose_ = filter_.updateFilter(currentOdometry_, currentScan_,
                                            map_);//, v_, omega_, utime_); //remove last 3 args for odo
