...
public:
    ...
    */
    ...
    /**
     * Convert from map (floats) to grid indices (int)
     */
    int mapToGridX(float x) const {return static_cast<int>((x - globalOrigin_.x) * cellsPerMeter_);}
    int mapToGridY(float y) const {return static_cast<int>((y - globalOrigin_.y) * cellsPerMeter_);}
    float gridToMapX(int x) const {return x * metersPerCell_ + globalOrigin_.x;}
    float gridToMapY(int y) const {return y * metersPerCell_ + globalOrigin_.y;}
